...
builtin.module  {
  builtin.func private @__torch__.TestModule.forward(%arg0: !torch.nn.Module<"__torch__.TestModule">, %arg1: !torch.tensor {torch.type_bound = !torch.vtensor<[2,3,?],f32>}) -> !torch.tensor {
    %1 = torch.operator "aten.sigmoid"(%arg1) : (!torch.tensor) -> !torch.tensor
...
note: see current operation: %1 = "torch.operator"(%0) {name = "aten.sigmoid"} : (!torch.tensor<[2,3,?],f32>) -> !torch.tensor
